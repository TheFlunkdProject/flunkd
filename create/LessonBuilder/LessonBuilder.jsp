...
function eraseJSP() {
	if (window.location.href == "file:///C:/Users/pears22/Desktop/GitHub/flunkd/create/LessonBuilder/LessonBuilder.html")
	{
...
<!-- To select a topic to write for: -->
	<div id="pathBanner">
		Course:
		<select name="courseName" id="courseName" onchange="document.forms['lessontext'].submit()" class="darkInput">
		...
		if (request.getParameter("courseName") != null) //carry over through form submission
			{
			String hCourse = request.getParameter("courseName");
			%>
			<script>
		...
		if (request.getParameter("topicName") != null)
			{
			String hTopic = request.getParameter("topicName");
			%>
			<script>
		...
		%>
		Lesson Type:
		<select name="lessonType" id="lessonType" onChange="updateTopic()" class="darkInput">
			<option value="TextLesson" selected="selected">Text Lesson</option>
			<option value="VideoLessons">Video Lesson</option>
			<!-- Carry past submissions: -->
			<script>
			document.getElementById('lessonType').value = "<%=request.getParameter("lessonType")%>";
			</script>
		</select>
	</div>
			...
			<!-- The following will execute if the user clicks "create new image" -->
				...
				if (request.getParameter("createNewImage") != null)
					...
					if (newImageName != null)
						...
						"callback_url=http://www.learningflare.com/create/LessonBuilder/newImage.jsp&ff_cancel_" + 
						"url=http://www.learningflare.com/create/LessonBuilder/close.jsp&ff_lang=en-US");
						}
						</script>
						<%
						}
		...
		<!-- The following will execute if the user clicks "Create New Graph" -->
			...
			before sending it to Fotoflexer. */
					...
					if (window.location.href != "file:///C:/Users/pears22/Desktop/GitHub/flunkd/create/LessonBuilder/LessonBuilder.html")
						...
						<!-- and then open the new window with the API to edit it -->
						window.open("http://fotoflexer.com/API/API_Loader_v1_01.php" + 
						"?ff_image_url=http://learningflare.com/create/LessonBuilder/newImages/defaultGraph500.png&ff_" + 
						"callback_url=http://www.learningflare.com/create/LessonBuilder/newGraph.jsp&ff_cancel_" + 
						"url=http://www.learningflare.com/create/LessonBuilder/close.jsp&ff_lang=en-US");
						}
					</script>
					<%
...
if (request.getParameter("preview") != null)
{
	...
	File fileLeft = new File("/home/learnfla/tomcat/webapps/learningflare.com/ROOT/create/LessonBuilder/TextLessonPreviewL.txt");
	...
	PrintWriter pwl = new PrintWriter(new FileOutputStream("/home/learnfla/tomcat/webapps/learningflare.com/ROOT/create/LessonBuilder/TextLessonPreviewL.txt"));
	...
	File fileRight = new File("/home/learnfla/tomcat/webapps/learningflare.com/ROOT/create/LessonBuilder/TextLessonPreviewR.txt");
	...
	PrintWriter pwr = new PrintWriter(new FileOutputStream("/home/learnfla/tomcat/webapps/learningflare.com/ROOT/create/LessonBuilder/TextLessonPreviewR.txt"));
	pwr.println(righttext);
	pwr.close();
	%>
<script>
if (window.location.href != "file:///C:/Users/pears22/Desktop/GitHub/flunkd/create/LessonBuilder/LessonBuilder.html")
window.open('http://www.learningflare.com/create/LessonBuilder/newfolder/TextLesson/TextLesson.jsp', 'Preview');
</script>
<%
}
%>
				...
				<%
				if (request.getParameter("save") != null && request.getParameter("topicName") != null) {
					String courseName = request.getParameter("courseName");
					String topicName = request.getParameter("topicName");
					String lessonType = request.getParameter("lessonType");
					String pathBoth = appPath + "/" + courseName + "/" + topicName + "/" + 
					lessonType + "/" + lessonType + "_pears22";
					String pathL = pathBoth + "/L.txt";
					String pathR = pathBoth + "/R.txt";
					File fileL = new File(pathL);
					File fileR = new File(pathR);
					
					if (fileL.exists() && fileR.exists()) {
						%><!-- First we write to L.txt:--><%
						StringBuffer lefttext = new StringBuffer(request.getParameter("leftpage"));
						int locl = (new String(lefttext)).indexOf('\n');
						while(locl > 0)
							{
						...
						PrintWriter pwl = new PrintWriter(new FileOutputStream(pathL));
						pwl.println(lefttext);
						pwl.close();
						
						%><!-- Now we write to R.txt:--><%
						StringBuffer righttext = new StringBuffer(request.getParameter("rightpage"));
						int locr = (new String(righttext)).indexOf('\n');
						while(locr > 0)
							{
						...
						PrintWriter pwr = new PrintWriter(new FileOutputStream(pathR));
						pwr.println(righttext);
						pwr.close();
					}
				}
